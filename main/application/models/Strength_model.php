...
class Strength_model extends CI_model{
	...
	public function get_strength_student_detail($strength_student_id){
		...
		if($sql->num_rows() > 0){
			...
			foreach($strength_student as $ss){
				...
				$ss->strength_student_detail_review = $this->get_strength_student_detail_review_by_strength_student_detail_id($ss->strength_student_detail_id)->result();
				foreach($ss->strength_student_detail_review as $ssdr){
					$ssdr->teacher_name = $this->user_model->get_user($ssdr->user_id)->first_row()->full_name;
	...
	public function get_strength_student_detail_review_by_strength_student_detail_id($strength_student_detail_id){
		$array = array('ssdr.strength_student_detail_id' => $strength_student_detail_id);
        $query = $this->db->select("*")
