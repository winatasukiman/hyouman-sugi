...
class Strength extends CI_Controller {
	...
	public function details($strength_student_id) {
		...
		$strength_student_detail = $this->strength_model->get_strength_student_detail($strength_student_id);
	...
	public function save_strength_student_detail(){
		...
		$this->strength_model->update_strength_student_detail($data,$this->input->post("strength_student_detail_id"));
		// Delete file in dbase if exist
		$this->strength_model->delete_strength_student_detail_file_by_strength_student_detail_id($this->input->post("strength_student_detail_id"));
		// UPLOAD FILE
		$files = $_FILES;
		$cpt = count($_FILES['reference']['name']);
		for($i=0; $i<$cpt; $i++){
			...
			if (!$this->upload->do_upload('reference')) {
				...
				$this->strength_model->insert_strength_student_detail_file($data);
			}
		}
