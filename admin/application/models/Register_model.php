...
class Register_model extends CI_model{
	...
	public function check_phone_number($ccd,$phone_number) {
		$array = array('country_code' => $ccd,'phone_number' => $phone_number,'user_type_id' => 4);
	...
	public function check_email($email) {
		$array = array('email' => $email,'user_type_id' => 4);
		return $this->db->select('*')
                ->from('user')
                ->where($array)
                ->get();
	...
	public function login($email, $pass) {
        $where_array = array('email'=> $email, 'password'=> $pass, 'user_type_id'=> 4);
