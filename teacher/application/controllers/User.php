...
class User extends CI_Controller {
    ...
    public function index() {
		...
		if ($sql->num_rows() > 0) {
			...
			$user_detail->date_of_birth = $this->change_date_format_from_db($user_detail->date_of_birth);
		}
	...
	public function change_date_format_from_db($datetime) {
        ...
        $new_Date = date("d-M-Y", strtotime($datetime)); //20 February 2020
        return $new_Date;
    }
	...
	public function add_user() {
		...
		if($check_phone_number_edit_profile->num_rows() > 0){
			...
			if (!$this->upload->do_upload('user_input_image')) {
				...
				$this->session->set_flashdata('category_success', 'Profile has been successfully Updated!');
				redirect('user');
			} else {
				...
				$id['user_id'] = $this->session->userdata('user_id');
				$this->user_model->update_user_data($data, $id);
				$this->session->set_flashdata('category_success', 'Profile has been uccessfully Updated!');
				redirect('user');
			}
		}
    }
